...
- Completed T017: added/expanded `tests/test_security_source_symlinks.py` with source-side symlink coverage for source repo cache paths and ancestors, git metadata paths, skills roots, skill folders, SKILL.md files, and symlinked source tree entries across `ensure_source_repo`, `ensure_source_repos`, `build_source_catalog`, `parse_skill_file`, and `add_project_skill`.
