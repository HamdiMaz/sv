...
Generated source indexes also record each skill's executable file bits as `executable_paths`, using paths relative to the skill folder. This keeps indexed installs fast while preserving scripts that were committed executable in Git; `sv add`, `sv sync`, and `sv update` apply those bits before validating the indexed content hash.
