...
def test_build_source_catalog_from_backend_rejects_invalid_explicit_jobs(
    tmp_path: Path,
) -> None:
    paths = SvPaths.from_home(tmp_path)
    repo = RepoConfig(id="Org/Skills", url="https://github.com/Org/Skills.git")
    ...
    with pytest.raises(SvError, match="SV_JOBS must be an integer between 1 and 64"):
        build_source_catalog_from_backends([repo], paths, {repo.id: ()}, jobs=0)
...
def test_build_source_catalog_skips_invalid_skills(tmp_path: Path):
    ...
    catalog = build_source_catalog([repo], paths, warn=warnings.append)
